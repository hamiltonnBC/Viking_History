...
const ROUTE_TYPES: { type: string; label: string; color: string; description: string }[] = [
  {
    type: 'raid',
    label: 'Raid Route',
    color: 'var(--blood-bright)',
...
export function MapGuide({ isVisible, onClose }: MapGuideProps) {
  return (
              ...
              {ROUTE_TYPES.map(item => (
                <div key={item.type} className="map-guide-item">
                  <svg width="32" height="32" viewBox="0 0 32 6" className="map-guide-route-swatch">
                    <line x1="0" y1="3" x2="32" y2="3" stroke={item.color} strokeWidth="3" strokeDasharray="6 3" />
                  </svg>
                  <div className="map-guide-item-text">
                    <strong style={{ color: item.color }}>{item.label}</strong>
                    <span>{item.description}</span>
